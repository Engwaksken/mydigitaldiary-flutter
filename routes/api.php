...
Route::middleware(['auth:sanctum', 'mobile.idempotent'])->name('api.')->group(function () {
    ...
    Route::put('profile/social-media/accounts/{account}/automatic-publishing', [SocialMediaAccountController::class, 'updateAutomaticPublishing'])
        ->whereNumber('account')
        ->name('profile.social-media.accounts.automatic-publishing');
    ...
    */
    ...
    // POST alias is intentional for Flutter multipart updates. PHP reliably
    // populates uploaded files for multipart POST requests on shared hosting.
    Route::post('social-media-planner/{socialMediaPost}', [
        SocialMediaPlannerController::class,
        'update',
    ])->whereNumber('socialMediaPost')
      ->name('social-media-planner.update-multipart');
