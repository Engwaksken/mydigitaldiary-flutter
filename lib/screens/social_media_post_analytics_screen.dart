...
class SocialMediaPostAnalyticsScreen extends StatefulWidget {
  ...
  @override
  State<SocialMediaPostAnalyticsScreen> createState() => _SocialMediaPostAnalyticsScreenState();
}

class _SocialMediaPostAnalyticsScreenState extends State<SocialMediaPostAnalyticsScreen> {
  ...
  Future<void> _edit(String platform, Map<String,dynamic>? metric) async {
    final names=['views','reach','impressions','likes','comments','shares','saves','clicks','replies'];
    final c={for(final n in names) n:TextEditingController(text:_int(metric?[n]).toString())};
    final external=TextEditingController(text:(metric?['external_post_id']??'').toString());
    final ok=await showModalBottomSheet<bool>(
      context:context,isScrollControlled:true,showDragHandle:true,
      builder:(context)=>SafeArea(top:false,child:SingleChildScrollView(
        padding:EdgeInsets.fromLTRB(18,4,18,MediaQuery.viewInsetsOf(context).bottom+18),
        child:Column(crossAxisAlignment:CrossAxisAlignment.stretch,children:[
          Text('Update ${platform.replaceAll('_',' ')} performance',style:const TextStyle(fontSize:18,fontWeight:FontWeight.w800)),
          const SizedBox(height:14),
          ...c.entries.map((e)=>Padding(padding:const EdgeInsets.only(bottom:10),child:TextField(controller:e.value,keyboardType:TextInputType.number,decoration:InputDecoration(labelText:e.key.replaceAll('_',' ').toUpperCase())))),
          TextField(controller:external,decoration:const InputDecoration(labelText:'External Post ID (optional)')),
          const SizedBox(height:16),
          FilledButton(onPressed:()=>Navigator.pop(context,true),child:const Text('Save Performance')),
        ]),
      )),
    );
    if(ok==true){
      await _service.updateAnalytics(postId:_postId,platform:platform,views:_int(c['views']!.text),reach:_int(c['reach']!.text),impressions:_int(c['impressions']!.text),likes:_int(c['likes']!.text),comments:_int(c['comments']!.text),shares:_int(c['shares']!.text),saves:_int(c['saves']!.text),clicks:_int(c['clicks']!.text),replies:_int(c['replies']!.text),externalPostId:external.text.trim());
      await _load();
    }
    for(final x in c.values){x.dispose();} external.dispose();
  }

  @override
  Widget build(BuildContext context){
    final post=_data['post'] is Map ? Map<String,dynamic>.from(_data['post'] as Map) : widget.post;
    final raw=_data['metrics'];
    final metrics=raw is List ? raw.whereType<Map>().map((e)=>Map<String,dynamic>.from(e)).toList() : <Map<String,dynamic>>[];
    ...
    return Scaffold(
      ...
      body:RefreshIndicator(onRefresh:_load,child:ListView(physics:const AlwaysScrollableScrollPhysics(),padding:const EdgeInsets.fromLTRB(16,12,16,80),children:[
        ...
        ...platforms.map((platform){final m=by[platform]; return Card(child:Padding(padding:const EdgeInsets.all(14),child:Column(crossAxisAlignment:CrossAxisAlignment.stretch,children:[
          ...
          const SizedBox(height:8),Text('Engagement: ${(m?['engagement_rate']??0)}%',textAlign:TextAlign.right,style:const TextStyle(fontSize:11,fontWeight:FontWeight.w700)),
        ])));}),
      ])),
    );
  }
