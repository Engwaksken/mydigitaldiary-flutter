...
class _PersonalisationScreenState extends State<PersonalisationScreen> {
  ...
  static const _notifyOptions = <String, List<String>>{
    'goal_progress': [
      ...
      'Only when an important goal needs attention'
    ],
    'monthly_review': [
      'Monthly review ready',
      'Know when your new monthly review is available'
    ],
    'finance_insights': [
      'Finance insights',
      'Helpful spending, saving and financial-health nudges'
    ],
    'productivity_nudges': [
      'Productivity nudges',
      'Occasional next-best-action reminders'
    ],
    'spiritual_insights': [
      'Spiritual Growth insights',
      'Allow spiritual reflection prompts in insights'
    ],
    'daily_affirmations': [
      'Daily affirmations',
      'Show a fresh motivational affirmation in Today’s Insight rotation'
    ],
    'subscription_reminders': [
      'Subscription reminders',
      'Important trial and subscription expiry reminders'
    ],
  };
  static const _focusOptions = <String, String>{
    'money': 'Manage my money',
    'day': 'Organise my day',
    'goals': 'Reach my goals',
    'health': 'Improve my health',
    'work': 'Manage work/business',
  ...
  Future<void> _load() async {
    try {
      ...
      setState(() {
        _ai
          ..clear()
          ..addAll((data['ai_data_permissions'] as List? ?? const [])
              .map((e) => e.toString()));
        _focuses
          ..clear()
          ..addAll((data['onboarding_focuses'] as List? ?? const [])
              .map((e) => e.toString()));
    ...
    return Scaffold(
      ...
      body: _loading
          ...
          : ListView(
              ...
              children: [
                Text('What should My Digital Diary help you with most?',
                ...
                const Text(
                    'Choose the areas you care about most. This helps us prioritise guidance and shortcuts.',
                  ...
                  children: _focusOptions.entries
                      .map((entry) => FilterChip(
                            ...
                            onSelected: (selected) => setState(() => selected
                                ? _focuses.add(entry.key)
                                : _focuses.remove(entry.key)),
                          ))
                      .toList(),
                ),
                const SizedBox(height: 26),
                Text('What may AI Planner use?',
                    style: Theme.of(context)
                        .textTheme
                        .titleMedium
                        ?.copyWith(fontWeight: FontWeight.bold)),
                const SizedBox(height: 6),
                const Text(
                    'You stay in control. Turn off any part of your diary you do not want included in AI context.',
                    style: TextStyle(color: Colors.black54)),
                ...
                const SizedBox(height: 6),
                const Text(
                    'Choose the helpful nudges you want. Essential security and account messages remain enabled.',
                    style: TextStyle(color: Colors.black54)),
                const SizedBox(height: 8),
                ..._notifyOptions.entries
                    .map((entry) => SwitchListTile.adaptive(
                          contentPadding: EdgeInsets.zero,
                          value: _notifyPrefs[entry.key] ?? true,
                          title: Text(entry.value.first,
                              style:
                                  const TextStyle(fontWeight: FontWeight.w600)),
                          subtitle: Text(entry.value.last),
                          onChanged: (value) =>
                              setState(() => _notifyPrefs[entry.key] = value),
                        )),
