...
class SocialMediaReportScreen extends StatefulWidget {
  ...
  @override
  State<SocialMediaReportScreen> createState() => _SocialMediaReportScreenState();
}
...
class _SocialMediaReportScreenState extends State<SocialMediaReportScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final r = await _service.report(period: _period, platform: _platform, status: _status);
      if (!mounted) return;
      setState(() { _report = r; _loading = false; });
    } catch (e) {
      if (!mounted) return;
      setState(() => _loading = false);
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Could not load report: $e')));
    }
  }
      ...
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            ...
            Card(child: Padding(
              ...
              child: Column(children: [
                DropdownButtonFormField<String>(
                  ...
                  onChanged:(v){ if(v==null)return; setState(()=>_period=v); _load(); },
                ),
                ...
                DropdownButtonFormField<String>(
                  ...
                  onChanged:(v){ setState(()=>_platform=v??''); _load(); },
                ),
                const SizedBox(height:10),
                DropdownButtonFormField<String>(
                  ...
                  items: const [
                    ...
                    DropdownMenuItem(value:'ready_to_share',child:Text('Ready to Post')),
            ...
            const Text('Platforms',style:TextStyle(fontSize:16,fontWeight:FontWeight.w800)),
            const SizedBox(height:8),
            ...const {'instagram':'Instagram','facebook':'Facebook','x':'X (Twitter)','tiktok':'TikTok','linkedin':'LinkedIn','whatsapp_status':'WhatsApp Status','whatsapp_channel':'WhatsApp Channel'}.entries.map(
              (e)=>Card(child:ListTile(title:Text(e.value),trailing:Text(_int(platforms[e.key]).toString(),style:const TextStyle(fontWeight:FontWeight.w800))))
            ),
            const SizedBox(height:18),
            Text('Post History (${posts.length})',style:const TextStyle(fontSize:16,fontWeight:FontWeight.w800)),
            const SizedBox(height:8),
            ...posts.map((post)=>Card(child:ListTile(
              leading:const Icon(Icons.campaign_outlined),
              title:Text((post['title']??'Untitled').toString()),
              subtitle:Text('${(post['status']??'draft').toString().replaceAll('ready_to_share','Ready to Post').replaceAll('_',' ')}\n${post['scheduled_at']??''}'),
              trailing:const Icon(Icons.insights_outlined),
              onTap:()=>Navigator.of(context).push(MaterialPageRoute(builder:(_)=>SocialMediaPostAnalyticsScreen(post:post))),
            ))),
          ],
        ),
      ),
