...
class ExpenseService {
  ...
  Future<List<Expense>> list() async {
    ...
    return order
        ...
        .map((id) => Expense.fromJson(byId[id]!))
        .toList();
  }
  ...
  Future<Expense> create(Expense expense) async {
    ...
    try {
      ...
      return Expense.fromJson(
          {...body, 'id': localId, '_offline_pending': true});
    }
  }
  ...
  Future<Map<String, dynamic>> extractReceipt(
    ...
    final response = await _api.postMultipart('expenses',
        fileFieldName: 'receipt_file',
        fileBytes: bytes,
        fileName: fileName,
        contentType: contentType,
        fields: const {'receipt_extract': '1'});
  ...
  Future<void> bulkDelete(List<int> ids) =>
      _api.post('expenses/bulk-delete', {'ids': ids});
}
