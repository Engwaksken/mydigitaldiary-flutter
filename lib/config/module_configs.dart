...
final List<ModuleConfig> moduleConfigs = [
  ...
  const ModuleConfig(
    ...
    fields: [
      FieldConfig(name: 'source', label: 'Source', type: FieldType.text, required: true, hint: 'e.g. Acme Corp Salary'),
      FieldConfig(name: 'category', label: 'Category', type: FieldType.text, hint: 'e.g. Employment, Business, Gift'),
      FieldConfig(name: 'amount', label: 'Amount', type: FieldType.number, required: true),
      FieldConfig(name: 'frequency', label: 'Frequency', type: FieldType.select, required: true, options: [
        FieldOption('one_time', 'One-time'), FieldOption('daily', 'Daily'), FieldOption('weekly', 'Weekly'),
        FieldOption('monthly', 'Monthly'), FieldOption('annually', 'Annually'),
      ]),
      FieldConfig(name: 'received_at', label: 'Date Received', type: FieldType.date, required: true),
  ...
  const ModuleConfig(
    ...
    amountField: 'amount',
    fields: [
      FieldConfig(name: 'category', label: 'Category', type: FieldType.text, required: true, hint: 'e.g. Groceries, Rent'),
      FieldConfig(name: 'amount', label: 'Budgeted Amount', type: FieldType.number, required: true),
      FieldConfig(name: 'period', label: 'Period', type: FieldType.select, required: true, options: [
        FieldOption('weekly', 'Weekly'), FieldOption('monthly', 'Monthly'), FieldOption('annually', 'Annually'),
      ]),
      FieldConfig(name: 'month_year', label: 'Month/Year', type: FieldType.text, hint: 'e.g. 2026-08 — leave blank for an ongoing budget'),
      FieldConfig(name: 'notes', label: 'Notes', type: FieldType.textarea),
    ],
  ),
  const ModuleConfig(
    ...
    dateField: 'date',
    fields: [
      FieldConfig(name: 'type', label: 'Type', type: FieldType.select, required: true, options: [
        FieldOption('borrowed', 'Borrowed (I owe them)'), FieldOption('lent', 'Lent (they owe me)'),
      ]),
      FieldConfig(name: 'person_name', label: 'Person', type: FieldType.text, required: true),
      FieldConfig(name: 'amount', label: 'Amount', type: FieldType.number, required: true),
      FieldConfig(name: 'date', label: 'Date', type: FieldType.date, required: true),
      FieldConfig(name: 'due_date', label: 'Due Date', type: FieldType.date),
      FieldConfig(name: 'status', label: 'Status', type: FieldType.select, required: true, options: [
        FieldOption('outstanding', 'Outstanding'), FieldOption('paid', 'Paid'),
      ]),
      FieldConfig(name: 'notes', label: 'Notes', type: FieldType.textarea),
    ],
  ),
  const ModuleConfig(
    ...
    dateField: 'target_date',
    fields: [
      FieldConfig(name: 'name', label: 'Goal Name', type: FieldType.text, required: true, hint: 'e.g. Emergency Fund, New Laptop'),
      FieldConfig(name: 'target_amount', label: 'Target Amount', type: FieldType.number, required: true),
      FieldConfig(name: 'target_date', label: 'Target Date', type: FieldType.date),
      FieldConfig(name: 'status', label: 'Status', type: FieldType.select, required: true, options: [
        FieldOption('in_progress', 'In Progress'), FieldOption('completed', 'Completed'), FieldOption('paused', 'Paused'),
      ]),
      FieldConfig(name: 'notes', label: 'Notes', type: FieldType.textarea),
    ],
  ),
  const ModuleConfig(
    ...
    fields: [
      ...
      FieldConfig(name: 'amount', label: 'Amount', type: FieldType.number, required: true),
      FieldConfig(name: 'contributed_at', label: 'Date', type: FieldType.date, required: true),
      FieldConfig(name: 'notes', label: 'Notes', type: FieldType.textarea),
    ],
  ),
  const ModuleConfig(
    ...
    dateField: 'logged_at',
    fields: [
      FieldConfig(name: 'meal_type', label: 'Meal', type: FieldType.select, required: true, options: [
        FieldOption('breakfast', 'Breakfast'), FieldOption('lunch', 'Lunch'), FieldOption('dinner', 'Dinner'), FieldOption('snack', 'Snack'),
      ]),
      FieldConfig(name: 'food_items', label: 'Food Items', type: FieldType.textarea, required: true, hint: 'e.g. 2 eggs, toast, orange juice'),
      FieldConfig(name: 'calories', label: 'Calories', type: FieldType.number),
      FieldConfig(name: 'logged_at', label: 'Date', type: FieldType.date, required: true),
      FieldConfig(name: 'notes', label: 'Notes', type: FieldType.textarea),
    ],
  ),
  const ModuleConfig(
    ...
    fields: [
      FieldConfig(name: 'activity', label: 'Activity', type: FieldType.text, required: true, hint: 'e.g. Running, Weights, Yoga'),
      FieldConfig(name: 'duration_minutes', label: 'Duration (minutes)', type: FieldType.number, required: true),
      FieldConfig(name: 'intensity', label: 'Intensity', type: FieldType.select, required: true, options: [
        FieldOption('light', 'Light'), FieldOption('moderate', 'Moderate'), FieldOption('intense', 'Intense'),
      ]),
      FieldConfig(name: 'calories_burned', label: 'Calories Burned', type: FieldType.number),
      FieldConfig(name: 'performed_at', label: 'Date & Time', type: FieldType.datetime, required: true),
      FieldConfig(name: 'notes', label: 'Notes', type: FieldType.textarea),
    ],
  ),
  const ModuleConfig(
    ...
    fields: [
      FieldConfig(name: 'sleep_date', label: 'Date', type: FieldType.date, required: true),
      FieldConfig(name: 'bed_time', label: 'Bed Time', type: FieldType.text, hint: 'e.g. 22:30 (24-hour HH:MM)'),
      FieldConfig(name: 'wake_time', label: 'Wake Time', type: FieldType.text, hint: 'e.g. 06:30 (24-hour HH:MM)'),
      FieldConfig(name: 'quality', label: 'Quality', type: FieldType.select, required: true, options: [
        FieldOption('poor', 'Poor'), FieldOption('fair', 'Fair'), FieldOption('good', 'Good'), FieldOption('excellent', 'Excellent'),
      ]),
      FieldConfig(name: 'notes', label: 'Notes', type: FieldType.textarea),
    ],
  ),
  const ModuleConfig(
    ...
    fields: [
      FieldConfig(name: 'checkup_type', label: 'Type', type: FieldType.text, required: true, hint: 'e.g. Dental, General, Eye'),
      FieldConfig(name: 'checkup_date', label: 'Checkup Date & Time', type: FieldType.datetime, required: true),
      FieldConfig(name: 'doctor_name', label: 'Doctor / Clinic', type: FieldType.text, hint: 'e.g. Dr. Smith, City Dental Clinic'),
      FieldConfig(name: 'next_due_date', label: 'Next Checkup Due', type: FieldType.datetime, hint: 'Leave blank if none scheduled yet'),
      FieldConfig(name: 'findings', label: 'Findings / Notes', type: FieldType.textarea),
    ],
  ),
  const ModuleConfig(
    title: 'Projects',
    ...
    dateField: 'deadline',
    fields: [
      FieldConfig(name: 'name', label: 'Project Name', type: FieldType.text, required: true, hint: 'e.g. Kitchen Renovation'),
      FieldConfig(name: 'status', label: 'Status', type: FieldType.select, required: true, options: [
        FieldOption('planned', 'Planned'), FieldOption('in_progress', 'In Progress'), FieldOption('on_hold', 'On Hold'), FieldOption('completed', 'Completed'),
      ]),
  ...
  const ModuleConfig(
    ...
    fields: [
      ...
      FieldConfig(name: 'status', label: 'Status', type: FieldType.select, required: true, options: [
        FieldOption('todo', 'To Do'), FieldOption('in_progress', 'In Progress'), FieldOption('done', 'Done'),
      ]),
      FieldConfig(name: 'due_date', label: 'Due Date', type: FieldType.date),
    ],
  ),
  const ModuleConfig(
    ...
    fields: [
      FieldConfig(name: 'title', label: 'Title', type: FieldType.text, required: true, hint: 'e.g. AWS Certification, MBA'),
      FieldConfig(name: 'level', label: 'Level', type: FieldType.select, required: true, options: [
        FieldOption('course', 'Course'), FieldOption('certificate', 'Certificate'), FieldOption('associate', 'Associate Degree'),
        FieldOption('bachelor', "Bachelor's"), FieldOption('master', "Master's"), FieldOption('phd', 'PhD'),
        FieldOption('bootcamp', 'Bootcamp'), FieldOption('other', 'Other'),
      ]),
      FieldConfig(name: 'status', label: 'Status', type: FieldType.select, required: true, options: [
        FieldOption('planned', 'Planned'), FieldOption('in_progress', 'In Progress'), FieldOption('completed', 'Completed'), FieldOption('on_hold', 'On Hold'),
      ]),
      FieldConfig(name: 'institution', label: 'Institution', type: FieldType.text),
      FieldConfig(name: 'start_date', label: 'Start Date', type: FieldType.date),
      FieldConfig(name: 'target_completion_date', label: 'Target Completion', type: FieldType.date),
  ...
  const ModuleConfig(
    ...
    fields: [
      FieldConfig(name: 'name', label: 'Name', type: FieldType.text, required: true),
      FieldConfig(name: 'relationship_type', label: 'Type', type: FieldType.select, required: true, options: [
        FieldOption('mentor', 'Mentor'), FieldOption('mentee', 'Mentee'), FieldOption('peer', 'Peer'),
        FieldOption('industry_contact', 'Industry Contact'), FieldOption('recruiter', 'Recruiter'), FieldOption('client', 'Client'), FieldOption('other', 'Other'),
      ]),
      FieldConfig(name: 'company', label: 'Company / Organization', type: FieldType.text, hint: 'e.g. Acme Corp'),
      FieldConfig(name: 'met_through', label: 'How You Met', type: FieldType.text, hint: 'e.g. Conference, LinkedIn'),
      FieldConfig(name: 'last_contact_date', label: 'Last Contact', type: FieldType.date),
      FieldConfig(name: 'next_follow_up_date', label: 'Next Follow-up', type: FieldType.date),
  ...
  const ModuleConfig(
    ...
    fields: [
      FieldConfig(name: 'name', label: 'Name', type: FieldType.text, required: true),
      FieldConfig(name: 'category', label: 'Category', type: FieldType.select, required: true, options: [
        FieldOption('family', 'Family'), FieldOption('work', 'Work'), FieldOption('friend', 'Friend'), FieldOption('romantic', 'Romantic'), FieldOption('other', 'Other'),
      ]),
      FieldConfig(name: 'relation_label', label: 'Relation Label', type: FieldType.text, hint: 'e.g. Sister, Best friend'),
      FieldConfig(name: 'priority', label: 'Priority', type: FieldType.select, required: true, options: [
        FieldOption('high', 'High'), FieldOption('medium', 'Medium'), FieldOption('low', 'Low'),
      ]),
      FieldConfig(name: 'last_meaningful_interaction', label: 'Last Meaningful Interaction', type: FieldType.date),
      FieldConfig(name: 'next_planned_interaction', label: 'Next Planned Interaction', type: FieldType.date),
      FieldConfig(name: 'strengthening_goal', label: 'Strengthening Goal', type: FieldType.textarea),
      FieldConfig(name: 'notes', label: 'Notes', type: FieldType.textarea),
    ],
  ),
  const ModuleConfig(
    ...
    fields: [
      FieldConfig(name: 'practice_type', label: 'Practice', type: FieldType.select, required: true, options: [
        FieldOption('prayer', 'Prayer'), FieldOption('meditation', 'Meditation'), FieldOption('scripture_reading', 'Scripture Reading'),
        FieldOption('worship', 'Worship'), FieldOption('fasting', 'Fasting'), FieldOption('service', 'Service / Volunteering'),
      ...
      FieldConfig(name: 'recurrence_frequency', label: 'Repeat', type: FieldType.select, options: [
        FieldOption('', 'Does not repeat'),
      ...
      FieldConfig(
        ...
        label: 'Repeat on (weekly only)',
        type: FieldType.text,
        hint: 'Use 1=Mon, 2=Tue, 3=Wed, 4=Thu, 5=Fri, 6=Sat, 7=Sun. Example: 1,3,5',
      ),
      FieldConfig(name: 'recurrence_ends_at', label: 'Repeat until (optional)', type: FieldType.date),
  ...
  const ModuleConfig(
    ...
    fields: [
      FieldConfig(name: 'category', label: 'Category', type: FieldType.select, required: true, options: [
        FieldOption('bug', 'Bug Report'), FieldOption('feature_request', 'Feature Request'), FieldOption('general', 'General'),
        FieldOption('complaint', 'Complaint'), FieldOption('compliment', 'Compliment'),
      ]),
      FieldConfig(name: 'subject', label: 'Subject', type: FieldType.text, required: true, hint: 'A short summary'),
      FieldConfig(name: 'message', label: 'Message', type: FieldType.textarea, required: true),
      FieldConfig(name: 'rating', label: 'Rating (1-5, optional)', type: FieldType.number),
    ],
  ),
];
